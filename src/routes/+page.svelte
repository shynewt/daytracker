<script lang="ts">
	...
	import {
		...
		exportJSON, importJSON, getStats
...
<svelte:window onkeydown={(e) => { if (e.key === 'Escape') { cancelRange(); popoverOpen = false; settingsOpen = false; } }} />
<div
...
<header class="h-12 shrink-0 flex items-center gap-2 px-4 bg-white dark:bg-zinc-900 border-b border-stone-200 dark:border-zinc-800 z-20">
	...
	<div class="ml-auto flex items-center gap-0.5">
		...
		<button onclick={exportJSON} class={iconBtn} title="Export JSON"><IconUpload size={17} /></button>
		<label class="{iconBtn}" title="Import JSON">
