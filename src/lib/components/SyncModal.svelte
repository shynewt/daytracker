<script lang="ts">
	...
	function handleManualJoin() {
		...
		const relay = localStorage.getItem('daytracker_relay_url') || 'wss://day-counter-relay.YOUR_SUBDOMAIN.workers.dev';
		// Manual code entry only has room code, no encryption key — generate QR data with empty key
		// Actually, manual join needs the full QR data or room code. Let's try parsing as JSON first,
		// then fall back to treating it as a room code.
		try {
			JSON.parse(manualCode.trim());
			handleJoin(manualCode.trim());
		} catch {
			// Treat as raw room code — can't work without key, show error
			joinStatus = 'error';
		...
		>
			...
			<div class="flex gap-1 mb-4 bg-stone-100 dark:bg-zinc-800 rounded-xl p-1">
				{#each [['host', 'Show QR'], ['join', 'Scan QR']] as [id, label]}
					<button
						...
						class="flex-1 py-1.5 text-sm font-semibold rounded-lg transition-colors {mode ===
						id
							? 'bg-white dark:bg-zinc-900 text-stone-900 dark:text-zinc-100 shadow-sm'
							: 'text-stone-400 dark:text-zinc-600 hover:text-stone-700 dark:hover:text-zinc-300'}"
						>{label}</button
					>
				{/each}
			</div>
			...
			{#if mode === 'host'}
				<div class="flex flex-col items-center gap-3">
					<div
						class="bg-white rounded-xl p-2 border border-stone-200 dark:border-zinc-700"
					>
					...
					{#if hostSession}
						<div class="text-center">
							<div
								class="font-mono text-lg font-bold tracking-[0.3em] text-stone-900 dark:text-zinc-100"
							>
								{hostSession.roomCode}
							</div>
							<div class="text-xs text-stone-400 dark:text-zinc-600 mt-1">
								Room code
							</div>
						</div>
						...
						<div class="flex items-center gap-2 text-sm">
							{#if hostSession.status === 'waiting'}
								...
								<span class="text-stone-500 dark:text-zinc-400"
									>{statusLabel[hostSession.status]}</span
								>
							...
							{:else if hostSession.status === 'error'}
								<span class="text-red-500"
									>{hostSession.error || 'Unknown error'}</span
								>
							{:else}
								<span class="text-stone-500 dark:text-zinc-400"
									>{statusLabel[hostSession.status]}</span
				...
				<div class="flex flex-col gap-3">
					{#if joinStatus === 'idle'}
						...
						<div class="flex flex-col gap-1.5">
							<span
								class="text-[10px] font-semibold uppercase tracking-wider text-stone-400 dark:text-zinc-600"
								>Or paste sync data</span
							>
