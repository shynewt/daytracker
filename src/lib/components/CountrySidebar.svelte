...
<aside class="flex flex-col overflow-y-auto w-full md:w-60 md:shrink-0 bg-white dark:bg-zinc-900 border-r border-stone-200 dark:border-zinc-800 {activeMobileTab !== 'countries' ? 'max-md:hidden' : ''}">
	...
	<div class="border-b border-stone-100 dark:border-zinc-800/60">
		...
		<div class="px-3 py-3 flex flex-col gap-2">
			<div class="relative">
				...
				<div class="absolute z-20 top-full left-0 right-0 mt-1 bg-white dark:bg-zinc-800 border border-stone-200 dark:border-zinc-700 rounded-xl shadow-lg overflow-hidden">
					...
					>
						<span class="text-base leading-none w-5 text-center">{getCountryFlag(c.code)}</span>
	...
	{#if addingCountry}
	<div class="px-3 py-3 flex flex-col gap-2 border-b border-stone-100 dark:border-zinc-800/60">
		<div class="relative">
			<input
	...
	{:else}
	<button onclick={() => addingCountry = true} class="m-3 mt-2 flex items-center justify-center gap-2 text-[13px] font-medium text-stone-400 dark:text-zinc-600 border border-dashed border-stone-200 dark:border-zinc-800 rounded-xl py-3 hover:text-amber-600 dark:hover:text-amber-400 hover:border-amber-400 dark:hover:border-amber-700 hover:bg-amber-50 dark:hover:bg-amber-950/20 transition-all">
		<IconPlus size={14} /> Add Country
	</button>
	{/if}
</aside>
